...
from transformers import AutoTokenizer, LEDConfig, LEDForConditionalGeneration, AutoModel
import numpy as np
import torch
import gc
import csv
...
def get_filtered_documents_universal_sentence_encoder(query, documents):
...
def get_bert_encodings(sentences):
...
def get_filtered_documents_bert(query, documents):
  query_embedding = np.array(get_bert_encodings([query]).cpu().detach())
  filtered_documents = []
  ...
  for document in documents:
    ...
    for sentence in sentences:
       sentence_embedding = get_bert_encodings(sentence).cpu().detach()[0, :]
